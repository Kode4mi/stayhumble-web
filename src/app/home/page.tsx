    ...
    <div>
      ...
      <div className="px-4 space-y-8 pt-4">
        {posts.map((post, index) => (
          <Post key={index} authorName={post.authorName} content={post.content} likes={post.likes} dislikes={post.dislikes} shares={post.shares} comments={post.comments} postTime={post.time} handleComment={() => handleComment(post)} />
        ))}
      </div>
    </div>
  );
